...
var sandbox = 1; // Whether sandbox is used
...
$(document).ready( function() {
			...
			success: function(d) {
				...
				$('#hitsList').children().fadeOut(500).promise().then(function() {
					...
					for (var i in hits) {
						...
						var hit = hits[i]; //actually an assignment
						var numAssignments = hits.length;
						for(var j = 0; j < numAssignments; j++){
							console.log("Found assignment " + j);

							var assignment = hit[j];
	...
	$("#yesSandbox, #noSandbox").on("click", function(){
		...
		statusbar.innerHTML = "sandbox changed to " + sandbox;
	});
